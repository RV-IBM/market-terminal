                            ...
                            try:
                                ...
                                if url:
                                    ...
                                    if res.status_code == 200:
                                        ...
                                        # 2. ULTRA-AGGRESSIVE REGEX EXTRACTOR
                                        clean_text = str(raw_prediction).strip()
                                        import re, ast, json
                                        # Strip AI reasoning logic (if any)
                                        clean_text = re.sub(r'<think>.*?</think>', '', clean_text, flags=re.DOTALL).strip()
                                        
                                        # Force extract ONLY the JSON payload, cutting off "AI VERDICT: "
                                        json_str = ""
                                        ...
                                        def cyber_highlight(text):
                                            ...
                                            text = re.sub(r'(?i)(bullish|support|rebound|growth|outperform|buy|upside|momentum|resilience|recovery|positive|breakout|rally|accumulation)', r'<span style="color: #00ff88; font-weight:bold;">\1</span>', text)
                                            text = re.sub(r'(?i)(bearish|resistance|contraction|downgrade|sell|downside|risk|breakdown|drawdown|distribution|drop|trough)', r'<span style="color: #ff3333; font-weight:bold;">\1</span>', text)
                                            return text
                                        ...
                                        try:
                                            ...
                                            try:
                                                # Fix formatting if AI returned python-style dict with single quotes
                                                safe_json = json_str.replace("'", '"').replace("True", "true").replace("False", "false")
                                                json_obj = json.loads(safe_json)
                                            except:
                                                try:
                                        ...
                                        if json_obj and isinstance(json_obj, dict):
                                            # Handle nested vs flat AI structures
                                            ...
                                            if not isinstance(quant, dict): quant = {}
                                            
                                            # Safely locate support and resistance levels
                                            sup = quant.get("support_level", quant.get("primary_support", data.get("support_levels", "N/A")))
                                            res_lvl = quant.get("resistance_level", quant.get("primary_resistance", data.get("resistance_levels", "N/A")))
                                            
                                            if isinstance(sup, list) and len(sup) > 0: sup = sup[0]
                                            if isinstance(res_lvl, list) and len(res_lvl) > 0: res_lvl = res_lvl[0]
                                            
                                            # Format to dollars if possible
                                            try: sup = f"${float(sup):.2f}" 
                                            except: pass
                                            try: res_lvl = f"${float(res_lvl):.2f}" 
                                            except: pass
                                            ...
                                            if "delta_summary" in quant or "volatility_profile" in quant:
                                                with st.expander("🔬 Quantitative Telemetry", expanded=False):
                                                    ...
                                                    st.warning(cyber_highlight(str(quant.get("volatility_profile", "N/A"))))

                                        else:
                                            # Absolute Failsafe if JSON is completely broken
                                            fallback_html = f"<div style='background: #0a0e17; border: 1px solid #1e293b; border-radius: 8px; padding: 20px; font-family: \"Courier New\", Courier, monospace; color: #e2e8f0; line-height: 1.7; font-size: 1.05em;'>{cyber_highlight(json_str)}</div>"
